...
| [0004](ADR-0004-arquitectura-interna.md) | Arquitectura interna: separació UI/agent | Superseded by ADR-0021 (clàusula UI-via-localhost) | 2026-04-17 |
...
| [0013](ADR-0013-isolation-pattern.md) | Isolation Pattern | Superseded 2026-05-18 — disabled (brownfield) | 2026-04-18 |
...
| [0017](ADR-0017-logging-pipeline.md) | Logging pipeline (tracing-subscriber + appender) | Accepted | 2026-04-22 |
| [0018](ADR-0018-python-sidecar-packaging.md) | Python sidecar packaging: PBS + uv | Accepted | 2026-04-23 |
| [0021](ADR-0021-ui-served-via-loopback.md) | UI servida pel sidecar via loopback HTTP | Accepted (supersedeix ADR-0004) | 2026-06-28 |

## ADRs no escrits (treball ja implementat)

Aquestes decisions es van implementar sense ADR formal retroactiu:

- **ADR-0019** UI integration + state machine (Fase 1) — implementat (wizard/onboarding a `src/onboarding/` + `src-tauri/src/lib.rs`).
- **ADR-0020** Graceful shutdown + kill process tree (Fase 2) — implementat (`src-tauri/src/lifecycle.rs`: POST `/admin/system/shutdown` + grace + SIGKILL, group-kill Unix / `taskkill /T` Windows). Pendent: Windows Job Object atòmic (K-002).

## Font

Decisions basades en:
- original plan (not in template)
- 4 external audit reports independents (external audit inputs (not in template))
