...
| **C — Full rewrite** | Risc molt alt. server-nexe funciona i té usuaris reals. Big-bang migration = llarga finestra sense producte funcional. |
...
- Revisió arquitectònica adversarial assistida per IA per a peces grans
...
- Implementation plan §Fase 1 i §Fase 2 — notes de desenvolupament locals
- Fase 0 validation report — notes de desenvolupament locals
- server-nexe backend — referenciat com a peça externa (projecte separat, no part d'aquest repositori)
