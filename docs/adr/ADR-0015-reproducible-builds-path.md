...
**Date:** 2026-04-18
**Status:** Accepted — **Active baseline**, SLSA L3 deferred
**Decided by:** Jordi Goy
**Updated:** `SOURCE_DATE_EPOCH` + reproducible-build.sh added
...
- `src-tauri/.cargo/config.toml`:
  ...
  ```
- `scripts/reproducible-build.sh` (**active**):
  ```bash
...
- **Two builds on the same machine from the same commit produce the same `target/release/nexe-app` SHA-256.** Empirically verified 2026-04-18: two `cargo clean` → `reproducible-build.sh` cycles produced identical SHA-256 `91630e8feac0fb286e758718c1ca0d9de02f34c6a8f17ac3f5c2a2382656f088`.
