...
| Validate plugin_id lowercase | APFS cross-platform bug fix |
...
| Release binary hygiene | `[profile.release]` strip+lto+panic=abort |
| `cargo audit` gate al CI | `--deny warnings` + `.cargo/audit.toml` ignores |
| Plugin integrity SHA-256 | Cache O(1), walk+hash complet (ADR-0014) |
| Isolation Pattern (postMessage) | Firewall JS per IPC plugins (ADR-0013) |
| Rate limiting per-plugin | Token bucket 1000 req/s |
| `dragDropEnabled: false` | Prevenció XSS via File.path |
...
| Single-instance lock | Planned | `tauri-plugin-single-instance` |
...
| Plugin signatures Ed25519 | Planned | Substitució hash auto-attestat per signatura criptogràfica |
| Signing macOS + notarització | Planned | Requereix Apple Developer ID del publicador |
