...
`nexe-app` has used `tracing::*` macros throughout the Rust codebase since its
early versions. Configuration of the global subscriber has been via
`tracing_subscriber::fmt().try_init()` (stdout only, no file persistence).

Between Fase 0 (2026-04-19) and the v0.1.2 hardening pass (2026-04-22) the logger
configuration oscillated:

1. **Fase 0 (v0.1.0-fase0)** — `tauri-plugin-log` was added then removed
   because initializing it after `tracing_subscriber::fmt().try_init()`
   ...
   connected.
2. **Hardening pass (2026-04-22, v0.1.2-fase0-v2 candidate)** — a hardening
   hypothesis theorized that `tracing-subscriber` without the `tracing-log` feature
...
3. **Runtime verification (2026-04-22 12:30)** — the first human `pnpm tauri
   dev` after the v0.1.2-fase0-v2 tag produced:
   ```
...
The AI-assisted adversarial review produced no GUI session. The regression was
only observable by a human running the desktop app.

The bug invalidated a signed claim in the `v0.1.2-fase0-v2` tag body.
...
|---|---|
| **A — Revert the reactivation change, accept the conflict as known deferred debt** | The original motivation for reactivating `tauri-plugin-log` (release Windows users have no log file) remains valid. Accepting the debt means shipping a product that is harder to support. `tracing-appender` solves the same problem without the global-logger conflict. |
...
| **C — Formal interop layer (`tauri-plugin-log` with `tracing` feature + custom `Layered`)** | Was the original reactivation plan. Empirically broken (this ADR exists because of it). Even with theoretical fix, keeps two crates doing the job of one and preserves the risk of future regressions when either upstream changes its `set_logger` strategy. |
...
- The Fase 0 logging conflict closed for real: release Windows (`windows_subsystem="windows"`)
...
- `cargo tree | wc -l` decreases: `tauri-plugin-log` brought `fern`, `colored`,
  ...
  follow-up).
- Runtime-verified on macOS at commit-time (all runtime gates pass with file
  writes confirmed).
...
**Status 2026-04-22:** macOS runtime-verified 4/4. Windows ARM64 VM + Linux VM
human GUI gates are deferred
...
This ADR is also a retraction: the v0.1.2 documentation added interop-claim
...
boot) are not substitutable with unit/integration test counts. Automated review
passes cannot execute a windowed desktop process.
