...
ADR-0002 va decidir empaquetar el sidecar Python com a venv relocatable via `build-python-bundle.sh`. L'enfocament funciona a macOS però és **insostenible cross-platform**:
...
- Token auth via env var `NEXE_AUTH_TOKEN`
- Supervisor gestiona lifecycle + health check
...
- venv relocatable insostenible cross-platform
