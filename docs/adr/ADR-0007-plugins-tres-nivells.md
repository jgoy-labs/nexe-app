...
## Nota — Priorització plugin:// (Fase 0)

**Context:** la revisió adversarial assistida per IA va assenyalar que sofisticar el protocol `plugin://` (integrity, CSP, rate limit) **abans** de tancar el sidecar core podria ser un ordre de prioritats invertit per a un starter standard.
...
**Revisió:** quan el sidecar s'integri, validar que el token auth i el lifecycle graceful no requereixin canvis retroactius al handler.
