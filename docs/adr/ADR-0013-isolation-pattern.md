...
Previously deferred because the starter only exposes the `greet` demo command. Reactivated because:
...
- `get_auth_token(_args)` — no frontend payload; returns session Bearer token for sidecar HTTP/WS auth.
- `quit_app(_args)` — no frontend payload; triggers graceful_quit dialog via command.
...
| Custom JS firewall (postMessage style) | Already present for `postMessage` from plugins; doesn't cover IPC (`invoke`). Complementary, not alternative. |
