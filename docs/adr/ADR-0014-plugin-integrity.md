...
**Revisions:**
- **v2 2026-04-21** — TOCTOU mtime cache bypass detectat per auditoria multi-IA (finding C01 consolidat, 3/3 IAs + verificació empírica APFS). L'algoritme original basat en `CacheEntry { mtime: SystemTime }` era bypassable per edició in-place d'un fitxer existent (dir mtime no canvia). Substituït per re-hash a cada request.
