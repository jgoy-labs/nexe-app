...
## 1. Claim original (revisió adversarial assistida per IA)
...
> pot canviar o no estar encara registrat. Un fix previ de la navegació de
> l'iframe va intentar resoldre-ho però el "source check" segueix sent fràgil si
...
**Nota important:** aquest finding el menciona només una de les passades de revisió
per IA. Les altres revisions automàtiques (59 i 23 findings respectivament) no el
detecten, i no s'aporta codi d'exploit ni evidència empírica concreta.

---

## 2. Context del codi actual

Fitxer `src/main.js:129-138` després del fix de navegació de l'iframe a la Fase 0 (commit `aa19de8`):

```js
// Registra automàticament tots els iframes `plugin://` coneguts al firewall
// (source validation). El rag spike és l'únic actual; Fase 4 ampliarà.
//
...
`src/main.test.js` (`"plugin firewall — iframe contentWindow race"`).
...
**Però:** aquesta hypothesis **ja està mitigada** pel fix de navegació de
l'iframe (el `load` listener registra el nou contentWindow quan la navegació
acaba).
...
`"plugin firewall — iframe contentWindow race"`).
...
**Veredicte:** la race condition descrita per la revisió adversarial assistida per IA és un fenomen teòric del
cicle de vida iframe (contentWindow reemplaçat durant navegació) però NO és un
vector explotable al codi actual:

1. **DoS:** mitigat per registrar a `DOMContentLoaded` + `load` (fix de navegació de l'iframe).
