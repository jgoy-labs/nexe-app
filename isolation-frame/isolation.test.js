// Unit tests for the Isolation Pattern filter (ADR-0013 active).
...
function loadIsolationWithWindow() {
    const window = {};
    // B2: the `fetch_from_sidecar` validator uses `new URL(...)`
...
describe("Isolation filter", () => {
    ...
    // ALLOWED is Object.create(null); prototype lookups return undefined.
    ...
    // C02 — quit_app takes no payload and must pass the hook.
    ...
    it("accepts quit_app with no args", () => {
        expect(() =>
    ...
    //
    // B1 (2026-04-21): fix regex bypass via commented decoy.
    ...
    it("drift check rejects decoy comment hiding an extra command (B1 mutation vector)", () => {
        ...
        const poisonedLibRs = `
// security C25: registered commands.
// legacy decoy snippet (DO NOT REMOVE): generate_handler![greet, quit_app, get_auth_token, fetch_from_sidecar]
...
`;
    ...
    describe("fetch_from_sidecar (security C25)", () => {
        ...
        // ─────────────────────────────────────────────────────────────────
        // B2 (2026-04-21) — structural URL parse
        //
        // AI-assisted adversarial review (Phase 2 latent finding)
        ...
        it("B2 rejects userinfo bypass (4 adversarial PoC vectors)", () => {
