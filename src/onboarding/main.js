// Onboarding wizard — state machine + routing.
//
 ...
 *
 * Fase 4b: the optional HF token is INTENTIONALLY excluded — it is
...
export function saveState() {
  try {
    // Omit hfToken from disk persistence. Build a shallow copy
...
function loadState() {
  try {
    ...
    // Copy own keys only, skipping prototype-polluting keys (no Object.assign
    // on parsed JSON — __proto__ from localStorage could pollute the prototype).
 ...
 *
 * Bounds check, shape validation, cross-field
 * inconsistency guard.
 */
/** Reset the wizard to Welcome (step 1) with no model. Used by the resume guards. */
...
function _isValidSelectedModel(model) {
  return !!(model && model.engine && model.model_id && model.name);
}
...
export function normalizeStateOnLoad() {
  ...
  // The wizard resumes the step if valid (1..MAX_STEPS),
  ...
  // Validated with an AI-assisted adversarial review.
  ...
  // Validate persisted selectedModel shape (current format has engine + model_id + name).
  ...
  // show "undefined").
  if (state.selectedModel && !_isValidSelectedModel(state.selectedModel)) {
    state.selectedModel = null;
    state.downloadProgress = 0;
  }

  // Cross-field inconsistency guard. Steps 3..5 (download /
 ...
 * or the deadline elapses. Covers the race between DOMContentLoaded
...
async function _validateResumedModel() {
  ...
  try {
    ...
    // fetch_catalog failed — treat the resumed model as unverifiable and reset.
  }
  _resetToWelcome();
  saveState();
}
...
export async function initOnboarding() {
  ...
  // Regression #1 2026-05-25: on first_run the sidecar seeds
