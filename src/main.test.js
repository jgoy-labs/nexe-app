// Real tests for the plugin postMessage firewall.
...
describe("plugin firewall — Tauri IPC shape filter (empíric Windows ARM64)", () => {
...
describe("plugin firewall — source validation", () => {
...
// -----------------------------------------------------------------------------
// AI-assisted adversarial review — iframe contentWindow registry race condition.
//
// Hypotheses to test:
...
// during transition). This is already mitigated by also registering on the load event.
// -----------------------------------------------------------------------------
describe("plugin firewall — iframe contentWindow race", () => {
  ...
  it("H2 (mitigated): registering on the load event covers the new contentWindow", () => {
    // Simula el fix: registrar tant a DOMContentLoaded com a load.
    const iframe = {
      contentWindow: { _stage: "about:blank" },
    };
  ...
  it("H2 race window: message during transition (before load fires) is dropped — DoS confirmed", () => {
    // The specific race condition under review: the plugin sends postMessage inside its
