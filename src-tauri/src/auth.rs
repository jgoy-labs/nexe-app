//! Auth token baseline — local Zero-Trust sidecar authentication (2026-04-21).
//!
...
//! perform POST /admin/system/shutdown or queries to Qdrant/LLM.
//!
//! # Security — Bearer token never exposed to the webview
//!
...
/// Structural sidecar URL validation (2026-04-21).
///
/// The old `url.starts_with("http://127.0.0.1:")` accepted 4 bypass vectors
/// documented by an AI-assisted adversarial security review:
///
...
pub(crate) fn validate_sidecar_url(
    ...
    // Userinfo rejected — known adversarial-review bypass vector. RFC 3986 §3.2.1:
...
/// Pure helper for HTTP method validation (2026-04-21)
...
/// webview (see Security note above). The ApiKey is the credential the web UI uses to authenticate
...
pub(crate) async fn fetch_from_sidecar(
    ...
    // `port_state.get()` is a lock-free atomic read; the port may have
    ...
    // POST/PUT). Pure validation helper.
...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────────
    // `validate_sidecar_url` tests (2026-04-21)
    //
    // Adversarial security review (latent Phase 2 risk): detected 4 bypass vectors in the old
    ...
    /// The 4 adversarial-review PoC vectors (userinfo bypass) must return an error
    /// (none of them should pass `Ok(())`). With the old `starts_with`
    /// all of these passed (empirical adversarial-review verdict 2026-04-21).
    ///
    ...
    /// Purely userinfo case (real host is 127.0.0.1 + userinfo present):
    ...
    /// Correct sidecar URL (no userinfo, exact host, explicit port)
    ...
    /// `localhost` hostname (not literal IP) must be rejected. The sidecar
    ...
    /// IPv4-mapped IPv6 `[::ffff:127.0.0.1]` must be rejected.
    ...
    /// HTTPS scheme rejected. The sidecar is HTTP-only localhost.
    #[test]
    ...
    /// `expected_port = Some(p)` requires exact match. Different port
    ...
    /// Missing port rejected. `url::Url::port()` returns `None` when not
    ...
    /// Adversarial-review PoC vector 4: `http://127.0.0.1:65536/` port overflow.
    ...
    /// T5b rewritten for real mutation testing (2026-04-21).
    ///
    /// **Test history:**
    /// - Original version (before this refactor) was THEATRE: the test defined its own
    ...
    ///
    /// **Refactor:** the real logic was extracted to
    ...
    /// NOT allowlisted methods rejected. Mutation: if
    ...
    /// GET/HEAD/DELETE do not accept body. Mutation: if someone
    ...
    fn validate_sidecar_path_blocks_normalised_bypasses() {
        ...
        for path in bypasses {
            assert_eq!(
                ...
                Err("BLOCKED_PATH"),
                "path {path:?} should be blocked after path normalisation"
            );
        }
    }
