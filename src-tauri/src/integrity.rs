...
//!
//! B5 (2026-04-21): fix TOCTOU verify→serve double-read.
...
//!
//! B6: MAX_HASH_FILE_BYTES cap per file (10MB) and
//! MAX_HASH_TOTAL_BYTES combined cap (50MB) to prevent OOM via malicious
//! plugins or large sparse files.
//!
//! Only cache successes. Errors auto-retry on next request.
//! LRU cap (500 entries) to prevent OOM via different IDs.
//! dev=false to avoid friction each-edit; release=true mandatory.
//!
...
/// which eliminates the TOCTOU verify→serve (B5).
///
...
/// B5: **verify + load in one atomic snapshot**.
///
