...
// Global bounded pool for the plugin:// handler (prevents thread-bomb).
...
// Per-thread reentrancy counter — prevents stack overflow if a plugin
...
///
/// Z3 (2026-04-21): security review detected that error
...
pub(crate) fn plugin_protocol_handler<R: Runtime>(
    ...
    // Reentrancy guard — decrement via RAII Drop
    struct DepthGuard;
    ...
    // B5 (2026-04-21): GET — verify + load in ONE atomic
...
fn apply_common_headers(
    ...
    builder
        ...
        .header("X-Content-Type-Options", "nosniff")
        // Plugin document's own CSP: no unsafe-inline + media-src.
        .header(
            "Content-Security-Policy",
...
/// B3 (2026-04-21) — atomic pre-queue job counter.
///
...
/// B3 fix (2026-04-22) — RAII guard that decrements
...
/// B3 fix (2026-04-22) — atomic helper that tries to acquire a queue slot.
...
mod tests {
    ...
    // B3 (2026-04-21) — queue bound atomic CAS.
